...
namespace PrintMatic.Repository.Repository
{
    public class ProductRepository : GenericRepository<Product>, IProdduct
    {
        ...
        {
            return _context.Products.Include("Category").Include("AppUser").Where(x => x.Id == id && x.IsDeleted == false).Include(x => x.ProductSales).ThenInclude(x => x.Sale).Include(x => x.ProductPhotos).Include(x => x.ProductColors).Include(x => x.ProductSizes).FirstOrDefault();
        }
        ...
        {
            return _context.Products.Where(x => x.Id == id && x.IsDeleted == false).Include(x => x.ProductSales).ThenInclude(x => x.Sale).Include(x => x.ProductPhotos).Include(x => x.ProductColors).Include(x => x.ProductSizes).Include(x => x.Reviews).FirstOrDefault();
        }
